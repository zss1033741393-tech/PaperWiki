...
全部机械验证通过：**W1 双向链接 19/19**、W2 实体一致、**W3 幂等（重跑仍 1 页、笔记保全）**、W4 台账、W5 无静默覆盖。期间发现并修复两个持久化/命名问题：**F2**（实体名哈希 → 改英文约定）与 **F3 🔴**（wiki 论文页被 gitignore，导致沉淀根本不进 git、链接断裂——已从 `.gitignore` 移除，论文页现真正持久化，这是"durable wiki"能成立的前提）。

## 发现汇总

| 编号 | 严重度 | 一句话 | 状态 |
| --- | --- | --- | --- |
| F3 | 🔴 | wiki 论文页被 gitignore → 沉淀的知识+用户笔记不进版本库、链接断裂 | **已修复**（.gitignore 移除 `wiki/papers/*`）|
