...
论文没交代来路，但只用它之前的东西能拼出来。两条线当时都在：一条是单模型内部的连续推理，Coconut（Chain of Continuous Thought）已经证明可以不解码 token、直接把末层隐藏状态回灌当下一步输入，在隐空间里做 CoT；另一条是 KV cache 的复用和 prompt caching，本来是给单模型省重复计算的。
...
杀伤力在于：这不是某个数据集上掉几个点，而是方法最省的那个场景（同构白盒）恰好是"多智能体"最不增值的场景，而多智能体最增值的场景（异构黑盒）恰好是它结构上进不去的。顺带还有一个更隐蔽的反例：latent thought 里若带着一个自信的错误，因为它不解码，任何内容过滤器、人工审计或下游 agent 都看不见，TextMAS 至少会把这句错话摆在明面上。隐空间省掉的可读性，在需要审计的场景里是负资产。

### 顺着缺口，一个值得做的新方向

报告第 15 节把可解释性退化和安全无从检测列为头两条独立局限。顺着这个，一个不增量的方向是：给共享 cache 配一个可观测旁路——不解码成完整文本，但用轻量探针把里面的关键信息暴露成可审计的低维摘要。

- 它针对的缺口：现在 latent 通道是个黑盒 blob，对不对、安不安全只能等最后一次解码，甚至解码了也未必忠实（debug probe 只是相关观测）。
- 能借的相邻领域：mechanistic interpretability 的线性探针、hidden state 上的 sparse autoencoder、以及带类型的 IPC 通道思想。在共享 latent thoughts 上挂一组便宜的 read-head，专门读出几样东西：置信度、关键断言、某类错误信号（比如算术出错），全程不必把 cache 翻成人话。
- 第一个实验：在 Qwen3-4B GSM8K 上，训一个便宜的线性探针或 SAE，在最终解码之前就从共享 latent 里检测算术错误，看它能不能抓住 debug-probe 文本能抓的那批错。抓得住，就说明可以用一个轻量 sidecar 把"可审计性"补回来，而不用交回文本那笔开销。

和已有工作划界：这不是去做论文刻意回避的跨模型适配器（那是让异构模型能交接），而是在同构前提不变的情况下，把不可审计的 cache 变成"带一层廉价观测的类型化通道"。风险也得说：探针只能覆盖你预先想到的那几类信号，覆盖不到的错误仍然是黑的，它把黑盒变灰，不会变透明。
