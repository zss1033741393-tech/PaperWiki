...
> [!note] 关于这一节
> 用 PaperForge 的读法补几个第一遍没展开的角度：作者怎么想到这套做法、整篇真正押在哪条假设上、怎么花一周证伪、最强反例、以及一个非增量方向。是接着往深里读，不是打分。论文没明说、属于我推断或猜测的地方都会讲明。

### 作者是怎么走到"让 agent 不说话"的

论文没交代来路，但只用它之前的东西能拼出来。两条线当时都在：一条是单模型内部的连续推理，Coconut（Chain of Continuous Thought，arXiv:2412.06769，Hao 等，Meta，2024-12）已经证明可以不解码 token、直接把末层隐藏状态回灌当下一步输入，在隐空间里做 CoT；另一条是 KV cache 的复用和 prompt caching，本来是给单模型省重复计算的。（这条前置线索经检索核实，非凭记忆。）

把这两条摆到"多个 agent 其实是同一个本地模型的不同角色"这个场景里，一个问题会自己冒出来：既然 Planner 和 Critic 是同一个模型，它把内部状态翻成人话、下一个角色再把人话读回同一个模型，这一整趟"连续到离散再到连续"到底图什么？顺着这个问题，intra-agent 用 Coconut 式 latent rollout、inter-agent 直接交接 KV cache，就是两条现成线的自然合流。真正要现场解决的技术障碍只有一个：末层隐藏状态和输入 embedding 不在同一个分布，直接回灌会漂移，于是有了那个用伪逆做的、training-free 的对齐矩阵。所以这篇的跳跃不是发明新模块，而是看穿"同模型角色之间的文本交接是冗余序列化"，再补上对齐这一块。这条路不需要论文自己的结论当前提——作者真实动机未必如此。

### 整篇真正押在哪一条假设上

报告在第 12、15、16 节已经反复点到同一件事。想把它拎成承重的那一条：所有 agent 必须共享兼容的内部表示空间——同一模型、同一 tokenizer、同样的层数和 KV 格式。因为方法的本体就是把一个 agent 的逐层 K/V 原样塞给下一个 agent，这一步一旦模型不同就不成立。

这条假设的要害不在"实现麻烦"，而在它和标题打架。论文叫 Latent Collaboration in Multi-Agent Systems，但机制真正 work 的前提，是这些"多 agent"其实是同一个模型戴不同角色帽子。也就是说：越异构、越黑盒的 agent（几家不同厂商的模型拼起来，恰恰是"多智能体"最有价值的场景），LatentMAS 越用不上；一旦真异构，方法就退回到需要一个学出来的跨模型适配器，而这正是论文刻意不声称的东西。报告第 16 节其实已经承认了这个分叉，这里想强调的是它的代价：LatentMAS 展示的是"同模型多角色协同推理"，而它省下的文本交接开销，只在这个同构前提里才是净赚。

### 花一周就能把这条假设证伪的实验

论文的消融比的是 LatentMAS 对 TextMAS，但缺了一个更狠的对照：同一个模型、给等量的 latent 计算预算、把所有角色的 prompt 串起来一次连续 rollout，中间不设 agent 边界。

- 数据：Qwen3-4B，GSM8K 小子集，sequential 拓扑。
- 三条对比：完整 LatentMAS（多角色、cache 交接）；单模型跑等量 latent steps、一次性带上全部角色提示、不做交接；再加原来的 TextMAS 做锚。
- 量：准确率、墙钟时间、峰值显存都对齐 latent 预算来比。
- 判定：如果 LatentMAS 明显赢过"等预算单模型连续 rollout"，那 agent 边界和角色结构确实带来了协作增益；如果两者打平，那 LatentMAS 很大程度上就是"换了个多智能体包装的、预算更长的 latent CoT"，协作那部分是虚的。

这正是主表回答不了的问题：现在的 Single 基线用的是文本、不是等量 latent 预算，所以分不清收益来自"隐空间"还是来自"结构"。

### 如果要反驳，最强的反例长什么样

最狠的反例，是一类真正需要异构 specialist 的任务：比如既要一个强代码模型、又要一个不同底座的领域模型，两者 tokenizer 和 KV 格式都不一样。

在这种题上 LatentMAS 结构性地出不了场——两个模型的 cache 根本没法直接交接。它只有两条路：要么退回文本通信（那它相对 TextMAS 的全部优势当场归零），要么把你锁死在同一个模型家族里（那就放弃了 specialist 多样性，而多样性正是你要多智能体的理由）。对照之下，纯文本的方法（TextMAS，或按题选异构模型的 GoA）在这里反而畅通无阻。

杀伤力在于：这不是某个数据集上掉几个点，而是方法最省的那个场景（同构白盒）恰好是"多智能体"最不增值的场景，而多智能体最增值的场景（异构黑盒）恰好是它结构上进不去的。顺带还有一个更隐蔽的反例，而且这条不是我的臆测：把 KV cache 当通道，本身就是一个不经文本披露、就能在 agent 间传递敏感内容的 opaque channel——LCGuard（arXiv:2605.22786）正是冲着这个风险去做 safe KV sharing 的守卫，而"AI agent 借隐蔽信道串通"更早已被 Secret Collusion（arXiv:2402.07510）正式形式化。latent thought 里若夹了一个自信的错误、或一段本不该外传的内容，因为它不解码，任何内容过滤器、人工审计或下游 agent 都看不见，TextMAS 至少会把它摆在明面上。隐空间省掉的可读性，在需要审计的场景里是负资产。

### 顺着缺口，一个值得做的新方向

报告第 15 节把可解释性退化和安全无从检测列为头两条独立局限。顺着这个，最顺手的 follow-up 是给共享 cache 挂一个可观测旁路：不解码成文本，用轻量探针把关键信息读成可审计的低维摘要。

但按 PaperForge 的规矩先检索一遍，就会发现这个方向已经有人在做——这也正是这版补做要暴露的问题。LCGuard（arXiv:2605.22786）就是把 KV cache 当 opaque channel、专门做 safe KV sharing 的守卫；latent 推理的可解释性也已成线（Ulterior Motives 检测 continuous thought 里的 misaligned reasoning、以及 arXiv:2606.01243 对 latent reasoning 做干预）。所以"加个探针审计 cache"不是非增量方向，是正在进行的工作。我上一版把它当"新方向"，是漏了检索，这里更正。

真正还空着、也更难的一格，是把问题从"检测"推到"预防"。现有审计都是事后看一眼，而 Secret Collusion（arXiv:2402.07510）已经论证隐蔽信道很难被可靠检测——监控做得再好，更强的 steganography 也可能绕过去。于是一个更值得做的方向是：在设计 latent 交接这一步，就给它一个可证明的信道容量上界，把"最多能偷偷夹带多少信息"物理地限住，而不是靠事后监控去追。

- 它针对的缺口：检测是被动的、可被更强的 steganography 绕过；容量约束是主动的，给出的是"最坏能泄露多少比特"的界。
- 能借的相邻领域：信息论的 covert-channel capacity 与 rate-distortion，加上 KV cache 的低秩/压缩——把交接前的 cache 过一个有容量预算的瓶颈，既省显存，又顺带压掉可用的隐蔽带宽。
- 第一个实验：Qwen3-4B 上给跨 agent 的 cache 交接加一个可调容量瓶颈，测两样——任务准确率掉多少（有用信息的损失），以及一个受训去夹带 side-channel 的探针还能塞进多少比特（隐蔽带宽的上界）。找那个既保住准确率、又把隐蔽带宽压到近零的点。

诚实划界：auditing 那条（LCGuard 和上面几篇可解释性工作）我已核实确有人做；"把信道容量上界当设计目标"这条，目前检索没看到直接对应的工作，但不敢断言空白——按 PaperForge 的四类信息纪律，它标成"待进一步 novelty 核实的方向"，不是已确证的空白。
