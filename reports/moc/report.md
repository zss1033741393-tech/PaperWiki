...
报告的开放问题问了自适应 K、来源可信度、抽取式压缩、保真指标。顺着上面那条自相矛盾，一个不增量的方向是：把合并的目标从"压掉相似"翻转成"保住分歧"。
...
和已有工作划界：这不是调 K、换蒸馏器或加来源分（那还是在"压掉相似"的框架里打转）；它换掉的是压缩这件事的目的——从"节省上下文"变成"守住分歧"。风险也得说：NLI 判"附和还是冲突"本身可能出错，判错了要么漏合并省不了钱、要么误合并又把分歧抹了，这个分类器的可靠性是新的瓶颈。
