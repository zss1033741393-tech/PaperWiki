...
论文进一步综合出 lightweight tools、balanced CLI frameworks、multi-agent orchestrators、enterprise full-featured、scenario-verticalized/research-oriented 五种模式（来源观点；定位：第 6.2—6.6 节）。这些是人工解释性综合得到的设计组合中心，不是互斥类别、统计聚类或成熟度排行榜。表 14 的 dominant assignments 分别为：lightweight 15 个、balanced CLI 18 个、multi-agent 22 个、enterprise 7 个、scenario/research 8 个；但完整项目矩阵和分配证据没有公开下载位置，因此外部读者不能独立复算模式稳定性。对实际选型最有用的仍是问题框架：先确定目标的 complexity envelope——单步辅助、长时工作、多 agent 协调还是生产治理——再选一致的设计组合，而不是把这些比例当作行业基准（来源观点与精读校正；定位：第 3.4、6.1—6.6、7.1 节）。

这也补强了“脚手架越多不一定越好”：论文报告更深协调常与更显式的上下文服务共同出现，更强执行常与结构化治理共同出现，并把它们限定为描述性共现而非因果律（来源观点；定位：第 5 节与第 7.2—7.3 节）。不过精读发现表 10—12 的部分 support 数值无法按正文定义成立：container isolation 的边际比例为 0.31，交集 support 却写成 0.89；MCP-first 的边际为 0.143，交集 support 却写成 0.62。因而这里保留架构问题和作者报告的方向，不把共现强度视为已复核事实（精读核验；定位：表 6、7、10、12）。
...
4. **架构选择值得成组检查。**70 项目论文报告深层协调、显式上下文服务、工具扩展边界和治理机制反复共同出现；这给出了有用的选型问题，但原始矩阵未公开且部分 support 算术不一致，尚不能把组合强度或因果性能当作确定结论。
...
3. **“harness”一词的研究用途不同，而且会改变经验分母。**arXiv:2606.10106 把它作为需要严格判定的概念，arXiv:2604.18071 只把它当作非 LLM 工程层的宽泛工作标签。后者的纳入条件只要求约 500 LOC 或等同架构足迹，并暴露至少一种 infrastructure capability。固定 pipeline、机械 context handling 或无独立控制的项目可能被 T1—T4 排除，所以严格筛选不仅增加标签，还可能改变五维比例、共现和模式分布。
...
- 对 `claw-*` 等相关项目做谱系去重，并公开 70×字段矩阵后，表 10—12 的 support/confidence/lift 和表 14 的 dominant patterns 能否复算？这是 2604.18071 的复现门槛（精读问题；定位：第 3.4、5.1、6.1 节及附录 A）。
