...
顺着上一节那条核心缺口——why→how 被反复断言，却从没被操作化——一个方向是：把描述性的 Five-Ws taxonomy 变成一个通信代价感知的协议合成器（communication-cost-aware protocol synthesis），让 why→how 这张映射本身成为被合成、被学出来的对象，而不是人手设计好再归档。
...
诚实划界：先给 novelty 降温——"学或合成通信协议"本身是老话题。CP-AgentNet（arXiv:2503.17850）用生成式 agent 自动设计通信协议，arXiv:2511.09171 在 MARL 里学高效通信协议，更早的 emergent-communication 一脉更是专门在学协议。所以"合成协议"不是我的新提法。相对具体、这几篇没直接做的，是把综述那套 Five-Ws 的 why→how 当成合成的 spec 与目标函数（拿论文自己写下的 ΔJ 约束优化当 objective），而不是从零学一个黑箱协议。按 PaperForge 的纪律，这条算"已有协议学习方向里、以 Five-Ws 为 IR 的一个待核实变体"，不是全新方向。它也不是又一个在固定协议类里做优化的方法，或论文呼吁的统一 benchmark（那是评测不是合成）。风险也摆前头——协议文法的表达力和可搜索性要权衡，以及 why 到底能不能被客观形式化（正好又绕回上一节的 κ）。
