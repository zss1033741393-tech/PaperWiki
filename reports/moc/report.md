...
> [!note] 关于这一节
> 用 PaperForge 的读法补几个第一遍没展开的角度：作者怎么想到多阶通信、整篇真正押在哪条假设上、怎么花一周证伪、最强反例、以及一个非增量方向。是接着往深里读，不是打分。论文没明说、属于我推断或猜测的地方都会讲明。

### 作者是怎么走到"多阶通信"的

论文没交代来路，但拼得出来。当时固定拓扑的 MAS 默认只拼一阶邻居，而 G-Designer、AgentPrune 这类工作都在优化图本身（哪些边），没人动图上流的消息。与此同时，在 GNN 那边，"多跳感受野"是最基本的常识——一个节点从 k 跳邻域聚合信息，早就是标准操作。再加上 RAG 里那套"embedding 去重加小模型压缩"也很成熟。

把这几样摆一起，缺口自己就出来了：现有 MAS 本质上是一阶消息传递，GNN 早就多跳了，为什么不把消息的感受野也放宽？放宽会带来上下文膨胀这个显而易见的代价，而它又有一个现成的解——从 RAG 工具箱里拿 embedding 去重加小 LLM 压缩。所以 MOC 的跳跃不是发明新机制，而是把 GNN 的"感受野"和 RAG 的"上下文压缩"两件现成东西，接到固定 DAG 的 MAS 上。这条路不需要论文自己的结论当前提——作者真实动机未必如此。

### 整篇真正押在哪一条假设上

报告在成本审计和证据缺口里已经把"压缩省不省钱""保不保真"敲得很透。想把承重的那条单独拎出来：MOC 全部押在"embedding 余弦高相似"约等于"语义冗余、可以安全合并"上。STM 的价值主张就是"放宽证据覆盖、但靠合并相似消息把上下文压回去"，而合并的触发和合并候选的挑选，用的都是同一个 MiniLM 的余弦相似度。

要命的是这条假设和论文自己的动机例子直接打架。Figure 1 里，一阶 Critic 给 48–72 小时，二跳专家给 72–96 小时——多阶通信的全部意义，就是把这条"和中间节点不一致的远端证据"捞回来给目标节点看。可是这两条消息在文本上高度相似（都是关于留置针留置时长、都是"几十小时"的短答），embedding 余弦大概率把它们判成"相似对"而合并掉。余弦相似度分不清"这两条在附和"和"这两条长得像但结论相反"。于是 STM 很可能把 MOC 辛辛苦苦多跳捞回来的分歧，又用表面相似度重新抹平。报告第 6 节点到高相似不保证数字、否定词被保留；这里想更进一步：最容易被判相似而合并的，恰恰是那些表面像、关键处相反的证据，而那正是多阶通信本来要保护的东西。

### 花一周就能把这条假设证伪的实验

不用重训、不用改拓扑，直接拿现成的 STM 代码打这条假设。

- 数据：手工构一批消息对，全部是高 embedding 相似、但关键处语义相反的——数字对立（48–72 对 72–96）、带否定、代码边界条件不同、结论冲突。
- 做法：把这些对喂给 STM 的合并流程，看合并出来的那条消息有没有保住这个区别，还是被揉成一个模糊中间值、或直接选了余弦更高的一条。再顺便测：当正确答案恰好依赖那个被合并掉的细节时，最终准确率掉多少。
- 判定：如果 STM 在"高相似但冲突"的对上系统性地把区别抹掉，那"相似即可安全合并"就被证伪了——而且证伪的正是论文动机例子那类场景。

这件事正好补上报告第 6 节说缺的 message-fidelity 和 adversarial compression 测试。

### 如果要反驳，最强的反例长什么样

最狠的反例，就是把论文的 Figure 1 反过来用。

设目标节点的正确答案，恰恰取决于分辨两条文本上几乎一样、但数值或逻辑相反的上游证据。MOC 的多阶把两条都捞进视野（这一步是对的），但 STM 的相似度合并把它们判成重复，压成一条掺和过的消息（比如"大约 48–96 小时"，或者选了余弦更高的那个候选），冲突就此消失。目标节点连"这里本来有分歧要裁"都不知道了。

杀伤力在于：这不是 MOC 在某个数据集上掉几个点，而是放宽感受野加按相似度有损合并这个组合，会在"决策依赖细微差别"的证据上制造假共识——医疗剂量、代码边界、法律条件、数值阈值全是这一类。结果是 MOC 可能在它专门为之设计的"证据冲突"场景上，反而比朴素一阶更差，因为一阶至少不会主动把两条冲突证据揉成一条。

### 顺着缺口，一个值得做的新方向

报告的开放问题问了自适应 K、来源可信度、抽取式压缩、保真指标。顺着上面那条自相矛盾，一个方向是：把合并的目标从"压掉相似"翻转成"保住分歧"。

先校准 novelty，别重犯 latentmas 那条漏检索的错：这个"别把分歧当噪声消解掉"的立场本身已经有人明确提出——arXiv:2606.04223（Consensus is Strategically Insufficient）直接论证"多智能体一味降分歧的目标是不够的，分歧可能是真信号而非 agent 错误"，还提出把 reasoning trace 抽象成符号化的分歧状态；arXiv:2512.07132（DART）也在拿分歧做工具调度。所以"保住分歧"作为主张不是我的原创。这里能落到具体、且这两篇没覆盖的，是把它接到 MOC 的 STM 合并那一步：让压缩本身变成分歧感知的。

- 它针对的缺口：MOC 现在把压缩当成"省上下文"，于是相似就合并。但多阶通信真正有价值的信号不是"更多证据"，而是"更多分歧"——压缩本该保护分歧，而不是抹掉它。
- 能借的相邻领域：argument mining、stance detection、以及带来源的 provenance-preserving 摘要。用一个便宜的 NLI 模型把上游消息分成"真附和"（互相蕴含）和"似而不同"（表面像、实质冲突）：只合并前者；对后者不压，反而把它们保成一个显式的"分歧集"，带上各自来源，交给目标节点一个结构化的"这是共识、这是尚未解决的分歧"对象，而不是一坨揉好的文本。
- 第一个实验：把 STM 的相似度合并换成"蕴含才合并、冲突则保留"的策略，在上一节那个冲突集上测它能不能把 MOC 丢掉的准确率补回来，同时 token 成本别涨太多。

诚实划界：把分歧当信号这个大方向已有人论证（arXiv:2606.04223、2512.07132，经检索核实），本条不重复那个立场；具体贡献只是把它落到 MOC 的 STM 压缩步——用 NLI 区分"蕴含才合并、冲突则保成显式分歧集"，让多阶捞回来的分歧不被相似度合并重新抹平。这个针对 STM 的具体做法我没检索到直接对应，但按 PaperForge 的纪律，它算"已有'分歧即信号'方向下一个待核实的工程实例"，不是全新主张。风险也得说：NLI 判"附和还是冲突"本身可能出错，判错了要么漏合并省不了钱、要么误合并又把分歧抹了，这个分类器的可靠性是新的瓶颈。
