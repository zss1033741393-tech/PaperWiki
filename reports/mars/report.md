...
> [!note] 关于这一节
> 用 PaperForge 的读法补几个第一遍没展开的角度：作者怎么想到同行评审这套、整篇真正押在哪条假设上、怎么花一周证伪、最强反例、以及一个非增量方向。是接着往深里读，不是打分。论文没明说、属于我推断或猜测的地方都会讲明。

### 作者是怎么走到"同行评审"这套结构的

论文没交代来路，但拼得出来。当时 MAD（Multi-Agent Debate，arXiv:2305.14325，Du 等，2023-05，经检索核实）很流行，可它 O(m²) 的通信成本是公认的痛——每个 agent 每轮都要读完其他所有人的长回答再改写。同时，generator-verifier 的非对称、LLM-as-a-judge，这些"生成一个、再独立核一个"的框架也都成熟了。

再看学术同行评审这套人类制度：审稿人之间根本不互相通信，各审各的，一个 editor（meta）把意见汇总后再回给作者。它天生就是"不让所有人互相说话、却仍能纠错"的解法。一个被 MAD 的二次成本硌到、又熟悉 generator-verifier 的人，很自然会想：这些 agent 为什么要互相辩论？让它们各自独立 review、把汇总集中到一个 meta 不就行了。所以 MARS 的跳跃不是新推理算法，而是把一套现成的人类制度（星形评审）搬到 agent 通信拓扑上，再补两个防过度纠正的 prompt 细节。这条路不需要论文自己的结论当前提——作者真实动机未必如此。

### 整篇真正押在哪一条假设上

论文的核心主张是"agent 之间的直接通信对有效协作不是必需的"。它承重的那条假设，是这句话背后的更强版本：集中汇总一批独立 review，能拿回跨 reviewer 互动本来会贡献的一切。星形省下的那部分通信，只有在"它本来就是冗余"时才是白赚。

问题在于，MAD 里 reviewer 互看的价值往往不是重复彼此的判断，而是看到别人的推理会改变自己的判断——一个审稿人因为另一个人的角度，才意识到自己的反对站不住，或者两个人各看到一半、拼起来才发现的错误。MARS 的 reviewer 从不互看，meta 拿到的是一组互不影响的独立报告。所以真正的假设是"独立 review 的集中聚合等于互动式讨论的产物"。报告的局限里已经点到 reviewer 常同模型同 prompt、错误高度相关；这里想更进一步：就算你把 reviewer 换成不同模型，只要他们不互动，那种"因对话才浮现的错误"仍然抓不到。效率的赢面和这条假设是有张力的——越是需要互动才能纠错的题，星形省掉的就越不是冗余。

### 花一周就能把这条假设证伪的实验

论文比的是 MARS 和 MAD 的总体准确率打平、token 减半。但要打这条承重假设，得专门盯"互动到底有没有用"的那批题。

- 先用 MAD 跑一遍，记录每个 reviewer 在看到别人回答后有没有改变立场；切出"确实因互看而翻了答案"的子集。
- 在这个子集上跑 MARS（无互看，只有 meta 聚合），看它能不能靠集中聚合拿回同样的正确答案。
- 判定：如果在"互动起作用"的子集上 MARS 系统性掉分，那"直接通信不必需"就在它本该覆盖的场景里被证伪了——省 token 的代价是丢掉了只有对话才浮现的纠错。

这比主表更狠，因为主表把"互动无所谓"的题和"互动有用"的题混在一起平均，正好把差别抹平了。

### 如果要反驳，最强的反例长什么样

最狠的反例不在论文自己都承认不适合的"开放探索"任务上，而是打进它的主场——验证。

构造一个多步证明，作者的草稿有一个跨步骤的隐错：第 3 步用的假设，和第 5 步的用法其实不一致。让 reviewer 1 盯前半段（步骤都对）、reviewer 2 盯后半段（步骤也都对）。两人各自独立看，都会给"没问题"；meta 收到两份"看着都对"，于是接受。而在 MAD 里，reviewer 1 看到 reviewer 2 对第 5 步的解读，才可能反应过来它和自己看的第 3 步冲突。

杀伤力在于：这不是 MARS 在它自认不擅长的地方翻车，而是独立评审加集中聚合这个结构，对"跨局部、只有拼起来才可见"的错误有系统性盲区，而这类错误恰恰是验证任务里最危险的。星形拓扑省下的通信，正是本可以照亮这个盲区的那束光。

### 顺着缺口，一个值得做的新方向

报告的开放问题问了自适应停止、更好的置信度、异构 reviewer。顺着上面那条盲区，一个方向是：让拓扑本身按题自适应，把通信变成"按需触发"而不是二选一。

先把 novelty 说清楚，别重犯只凭直觉宣称"新"的错：按题自适应的稀疏拓扑已经不是空白。arXiv:2406.11776 就在给 MAD 做 sparse communication topology；2606.01828（Dynamic Trust-Aware Sparse Communication Topology）更是直接点出固定稀疏拓扑"没法随任务实例和中间推理状态调整"，做的正是动态化。所以"动态拓扑"本身是正在进行的方向，不能算我的新提法。这里还能站住的、相对具体的差别，只是把触发信号锚在上一节那个盲区上——不是按 trust 或图结构预优化，而是按"reviewer 是否在同一处出现实质分歧"来决定要不要开那条最小对话。

- 它针对的缺口：MARS 和 MAD 是拓扑光谱两个极端（全星形对全连接），而已有的动态方法多按 trust 或全局图结构来调；"按跨局部分歧触发"这个具体信号还少见。
- 能借的相邻领域：event-triggered communication（正好是《Five Ws》那篇综述里 when 的核心）、以及 active learning 的"只在不确定时才查询"。默认走 MARS 的廉价独立评审；只有当两个 reviewer 在同一处结论相反、或 meta 置信度偏低时，才在这两个 reviewer 之间开一条最小 pairwise 对话。O(m²) 的成本只花在稀疏那批真正需要它的题上，其余保持 O(m)。
- 第一个实验：给 MARS 加一个分歧或低置信检测器，只在触发时开 pairwise 对话，测它能不能把上一节那种"跨局部错误"子集的准确率补回来，同时 token 成本贴近原版 MARS，并和 2606.01828 那种 trust-aware 动态拓扑做对照。

诚实划界：动态稀疏拓扑这个大方向已有人做（2406.11776、2606.01828，经检索核实），本条不重复那个主张，只在其内收窄一个机制——用"跨局部分歧"而非 trust/图先验来触发、且只开 pairwise 而非重排整张图。这个具体触发信号我没检索到直接对应，但按 PaperForge 的四类信息纪律，它算"已有动态拓扑方向内一个待核实的变体"，不是全新方向。风险也得说：分歧检测本身可能不准，触发早了省不了钱、触发晚了漏掉该救的题，阈值得校。
