...
论文没交代来路，但只用它之前的东西能还原。当时 MoA（Mixture-of-Agents，arXiv:2406.04692，Together AI，2024-06，经检索核实）已经摆在那，毛病也清楚：每层把所有候选模型的完整回答拼给下一层，模型池一大就是成本、噪声和上下文三头涨，而且 many-to-one 的拼接压根表达不了"医学模型该影响通用模型、法律模型不该掺和"这种有向关系。与此同时，垂直专精模型越来越多（Coder、Mathstral、Biomedical、Finance、Saul 各一路），"到底该叫哪几个来"这个选择问题没人正经处理。
...
顺着上面——排名信号来自模型池内部的相互认可，天然逃不出共享偏见——一个方向是：把定边方向的信号从"模型互评"换成"可验证的外部信号"。
...
诚实划界：用 verifier 或 process reward model 去引导 LLM 推理，本身是很成熟的一片（test-time verification、verifier-guided decoding），所以"引入外部 verifier"这个零件不新。相对具体、我这次检索没撞到直接对应的，是把 verifier 用来定"图里谁听谁"的边方向——在一个异构模型编排里，用外部真值而非 peer 互评来决定改写顺序。按 PaperForge 的纪律，这条标成"在成熟的 verifier-guided 方向内、针对 GoA 边定向的一个待核实变体"，不是全新方向。局限也摆明：只有自带廉价 verifier 的任务（代码、数学）用得上，选择题、开放问答这类只能退回弱信号或混合信号。
