...
[[reports/agentmaster/report|AgentMaster: A Multi-Agent Conversational Framework Using A2A and MCP Protocols for Multimodal Information Retrieval and Analysis report]]
...
- [[llm-complexity-classification|LLM complexity classification]]
...
- [[fhwa-bridge-public-datasets|FHWA bridge public datasets]]
- [[llm-multi-agent-systems|LLM multi-agent systems]]
- [[a2a|A2A]]
- [[mcp|MCP]]
- [[tool-use|tool-use]]
- [[protocol-interoperability|protocol interoperability]]
- [[training-free-orchestration|training-free orchestration]]
...
2024 到 2025 年，agent 框架一下子多起来——AutoGen、CrewAI、LangGraph 之类——它们的共同毛病是把协作逻辑、工具调用、上下文管理全揉在 prompt 里，换个工具或加个 agent 就要动一大片。差不多同时冒出两个还很新的公开协议规范（是规范、不是论文）：Anthropic 的 MCP（2024 年底）管 agent 怎么访问工具和数据，Google 的 A2A（2025 年）管 agent 之间怎么发现和委派。这两个 spec 各自的适用范围其实是正交的，一个横向（agent 对 agent），一个纵向（agent 对资源）。
...
顺着上面那条缺口——协调器是个不可靠、又没被单独验证的单点——一个方向是：把协调器从"推理时一次 LLM 拍板"改成"先合成一份可静态检查的带类型计划，再执行"。
...
诚实划界：给 agent workflow 做形式化验证已经有人在做——arXiv:2606.06523（Lean4Agent）就在给 agent 的 workflow 和 trajectory 做形式建模与验证，另有一批 verification-centered 的 agentic workflow 工作。所以"验证 agent 计划"不是空白。相对具体、这几篇没直接覆盖的，是把契约挂到 A2A 消息本身（谁保证什么），并在执行前对协调器那份计划做静态检查（约束有没有被拆散、依赖有没有成环、类型对不对得上）——重点在编译期就拦住依赖型查询的丢约束，而不是运行时验证轨迹。按 PaperForge 的纪律，这条算"agent-workflow 验证方向内一个待核实的具体做法"，不是全新方向。它也不是再包一个 orchestrator 框架（AutoGen/LangGraph 运行时还是靠 LLM 编排），或换官方 SDK（那解决互操作，不是计划正确性）。风险也摆前头：计划形式化到什么粒度，才既能查错又不掐死 LLM 的灵活性，这个度得试。
