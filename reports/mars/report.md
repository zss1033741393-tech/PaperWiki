...
论文没交代来路，但拼得出来。当时 MAD（Multi-Agent Debate）很流行，可它 O(m²) 的通信成本是公认的痛——每个 agent 每轮都要读完其他所有人的长回答再改写。同时，generator-verifier 的非对称、LLM-as-a-judge，这些"生成一个、再独立核一个"的框架也都成熟了。
...
报告的开放问题问了自适应停止、更好的置信度、异构 reviewer。顺着上面那条盲区，一个不增量的方向是：让拓扑本身按题自适应，把通信变成"按需触发"而不是二选一。

- 它针对的缺口：MARS 和 MAD 其实是拓扑光谱的两个极端——全星形对全连接。真正该问的不是选哪个，而是"这道题到底需不需要互动"。
- 能借的相邻领域：event-triggered communication（正好是《Five Ws》那篇综述里 when 的核心）、以及 active learning 的"只在不确定时才查询"。默认走 MARS 的廉价独立评审；只有当 reviewer 之间出现实质分歧、或 meta 置信度偏低时，才在恰好那两个分歧的 reviewer 之间开一条最小的对话通道。O(m²) 的成本只花在稀疏的那批真正需要它的题上，其余全保持 O(m)。
- 第一个实验：给 MARS 加一个分歧或低置信检测器，只在触发时开 pairwise 对话，测它能不能把上一节那种"跨局部错误"子集的准确率补回来，同时 token 成本贴近原版 MARS。

和已有工作划界：这不是在 MARS 上调轮数或换更强 meta（还是固定星形），也不是退回 MAD（固定全连接）；它把两篇共享的"拓扑固定"假设整个拆掉，让通信量随题目的实际需要伸缩。风险也得说：分歧检测本身可能不准，触发早了省不了钱、触发晚了漏掉该救的题，这个阈值得校。
